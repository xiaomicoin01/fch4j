...
import org.freecash.utils.HttpClientCommpont;
...
public class FreeDriveComponnet {
    ...
    public FreeDriveGetResponse put(FreeDriveGetRequest request) throws Exception{
        ...
        HttpPost post = HttpClientCommpont.getPostForMap(getUrl,map);
        String res = HttpClientCommpont.send(post);
    ...
    private <T> T send(Object params, String url,Class<T> resultType)throws Exception{
        ...
        HttpPost post = HttpClientCommpont.getPostForJson(url,json);
        String res = HttpClientCommpont.send(post);
        ObjectMapper objectMapper = new ObjectMapper();
